...
contract GlowFunFactory_V2 is Ownable, ReentrancyGuard, Pausable {
    ...
    mapping(address token => mapping(address wallet => uint256 lastBuyTimestamp)) public lastBuyTime;

    mapping(address referrer => uint256 earned) public referralEarnings;

    mapping(address token => uint256 usdcAmount) public pendingGraduationUsdc;
    mapping(address token => uint256 tokenAmount) public pendingGraduationTokens;
    mapping(address token => address creator) public pendingGraduationCreator;
    mapping(address token => uint256 creatorUsdc) public pendingCreatorGraduationUsdc;

    address public kingOfHill;
    uint256 public kingOfHillRaised;

    mapping(address token => uint8 mask) private _milestoneMask;
    ...
    event TokensBought(
        ...
        uint256 protocolFee,
        uint256 referralFee,
        uint256 antiSnipeFee,
        uint256 newRealUsdcRaised,
        uint256 timestamp
    );
    event TokensSold(
        address indexed token,
    ...
    constructor(address _usdc, address _feeRecipient, address _graduationRecipient, address _owner) Ownable(_owner) {
        if (
            _usdc == address(0) || _feeRecipient == address(0) || _graduationRecipient == address(0) || _owner == address(0)
    ...
    function setAntiSnipeConfig(uint256 duration, uint256 taxBps) external onlyOwner {
        ...
        antiSnipeDuration = duration;
        antiSnipeTaxBps = taxBps;
    ...
    function blacklistWallet(address wallet, bool blacklisted) external onlyOwner {
    ...
    function getSellQuote(address token, uint256 tokensIn) external view returns (uint256 usdcOut) {
        ...
        uint256 fee = (gross * DEFAULT_PROTOCOL_FEE_BPS) / BPS_DENOMINATOR;
        usdcOut = gross - fee;
    }
    ...
    function getProgress(address token) public view returns (uint256) {
        ...
        if (progress > BPS_DENOMINATOR) return BPS_DENOMINATOR;
        return progress;
    }
    ...
    function _emitMilestones(address token, uint256 raised) internal {
        ...
        if (progress >= 2500 && (mask & 1) == 0) {
            _milestoneMask[token] = mask | 1;
            emit MilestoneReached(token, 2500, raised);
            mask = _milestoneMask[token];
        }
        if (progress >= 5000 && (mask & 2) == 0) {
            _milestoneMask[token] = mask | 2;
            emit MilestoneReached(token, 5000, raised);
            mask = _milestoneMask[token];
        }
        if (progress >= 7500 && (mask & 4) == 0) {
            _milestoneMask[token] = mask | 4;
            emit MilestoneReached(token, 7500, raised);
            mask = _milestoneMask[token];
        }
        if (progress >= 10_000 && (mask & 8) == 0) {
            _milestoneMask[token] = mask | 8;
            emit MilestoneReached(token, 10_000, raised);
        }
    }
