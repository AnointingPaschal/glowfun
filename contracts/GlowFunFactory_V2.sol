...
contract GlowToken is ERC20 {
    ...
    function updateMetadata(
        ...
        emit URIUpdated(string(abi.encodePacked('data:application/json;utf8,{"name":"', name(), '","image":"', imageUri, '"}')));
    }
    ...
    function _update(address from, address to, uint256 value) internal override {
        if (locked && from != address(0) && to != address(0) && from != factory && to != factory && from != creator && to != creator) 
...
contract GlowFunFactory_V2 is Ownable, ReentrancyGuard, Pausable {
    ...
    address[] public launchedTokens; // Replaced private array
    ...
    constructor(address _usdc, address _feeRecipient, address _graduationRecipient, address _owner) Ownable(_owner) {
        if (_usdc == address(0) || _feeRecipient == address(0) || _graduationRecipient == address(0) || _owner == address(0)) revert InvalidAddress();

        uint8 usdcDecimals;
        try IERC20Metadata(_usdc).decimals() returns (uint8 d) { usdcDecimals = d; } catch { revert InvalidToken(); }
    ...
    // ── Buy ───────────────────────────────────────────────────────────────
    function buyTokens(address token, uint256 minTokensOut, address referrer) external nonReentrant whenNotPaused returns (uint256 usdcIn, uint256 tokensOut) {
        ...
        _emitTokensBought(token, msg.sender, referrer, usdcIn, tokensOut, _priceFromReserves(state.virtualUsdcReserves, state.virtualTokenReserves), f, state.realUsdcRaised);
    ...
    // ── Sell ──────────────────────────────────────────────────────────────
    function sellTokens(address token, uint256 tokensIn, uint256 minUsdcOut) external nonReentrant whenNotPaused returns (uint256 usdcOut) {
        TokenState storage state = _activeTokenState(token);
        _checkBlacklist(token, msg.sender);
        ...
        _emitTokensSold(token, msg.sender, tokensIn, usdcOut, _priceFromReserves(state.virtualUsdcReserves, state.virtualTokenReserves), protocolFee, state.realUsdcRaised);
    }
    ...
    function updateTokenMetadata(address token, string calldata _imageUri, string calldata _description, string calldata _twitter, string calldata _telegram, string calldata _website) external {
        if (!isLaunchedToken[token]) revert NotLaunched();
        if (msg.sender != tokenStates[token].creator && msg.sender != owner()) revert InvalidAddress();
    ...
    function updateConfig(UpdateSettingsParams calldata s) external onlyOwner {
        ...
        creatorGraduationFeeBps = s.creatorGraduationFeeBps;
        referralFeeBps = s.referralFeeBps;
        antiSnipeDuration = s.antiSnipeDuration;
        antiSnipeTaxBps = s.antiSnipeTaxBps;
        maxBuyBps = s.maxBuyBps;
        buyCooldown = s.buyCooldown;
        creatorLockDuration = s.creatorLockDuration;
        perTokenGraduationFeeBps = s.perTokenGraduationFeeBps;

        emit ConfigUpdated();
    }

    function proposeFeeRecipient(address proposed) external onlyOwner { if (proposed == address(0)) revert InvalidAddress(); pendingFeeRecipient = proposed; emit FeeRecipientProposed(proposed); }
    function acceptFeeRecipient() external { if (msg.sender != pendingFeeRecipient) revert InvalidAddress(); feeRecipient = pendingFeeRecipient; pendingFeeRecipient = address(0); emit FeeRecipientUpdated(feeRecipient); }
    ...
    function emergencyUnpause() external { if (!paused() || block.timestamp < pausedAt + MAX_PAUSE_DURATION) revert PauseNotExpired(); pausedAt = 0; _unpause(); }
    ...
    function getTokenPrice(address token) public view returns (uint256) {
        if (!isLaunchedToken[token]) revert NotLaunched();
        return _priceFromReserves(tokenStates[token].virtualUsdcReserves, tokenStates[token].virtualTokenReserves);
    }
    ...
    function getBuyQuote(address token, uint256 usdcIn) external view returns (uint256 tokensOut) {
        ...
        uint256 snipeTax;
        if (block.timestamp < state.createdAt + antiSnipeDuration) snipeTax = (usdcIn * antiSnipeTaxBps) / BPS_DENOMINATOR;
        uint256 protocolFee  = ((usdcIn - snipeTax) * protocolFeeBps) / BPS_DENOMINATOR;
        tokensOut = _getBuyAmount(state.virtualUsdcReserves, state.virtualTokenReserves, usdcIn - protocolFee - snipeTax);
    ...
    function getProgress(address token) public view returns (uint256) {
        if (!isLaunchedToken[token]) revert NotLaunched();
        if (graduationThreshold == 0) return BPS_DENOMINATOR;
        uint256 progress = (tokenStates[token].realUsdcRaised * BPS_DENOMINATOR) / graduationThreshold;
    ...
    function isCreatorLocked(address token) external view returns (bool) {
        if (!isLaunchedToken[token]) revert NotLaunched();
        return tokenStates[token].creatorTokensLocked && block.timestamp < tokenStates[token].creatorLockExpiry;
    }

    // ── Internal Helpers ──────────────────────────────────────────────────
    function _emitTokenLaunched(address token, address creator, LaunchParams calldata p, LaunchAllocations memory a, uint256 _graduationThreshold) internal {
        emit TokenLaunched(token, creator, p.name, p.symbol, a.supply, a.curveTokens, a.graduationTokens, a.creatorTokens, _graduationThreshold, block.timestamp);
    }

    function _emitTokensBought(address token, address buyer, address referrer, uint256 usdcIn, uint256 tokensOut, uint256 price, BuyFees memory f, uint256 raised) internal {
        emit TokensBought(token, buyer, referrer, usdcIn, tokensOut, price, f.protocolFee, f.referralFee, f.snipeTax, raised, block.timestamp);
    }

    function _emitTokensSold(address token, address seller, uint256 tokensIn, uint256 usdcOut, uint256 price, uint256 protocolFee, uint256 raised) internal {
        emit TokensSold(token, seller, tokensIn, usdcOut, price, protocolFee, raised, block.timestamp);
    }
    ...
    function _deployToken(LaunchParams calldata p, uint256 supply) internal returns (address) {
        TokenMetadata memory meta;
        meta.name        = p.name; meta.symbol      = p.symbol; meta.description = p.description; meta.imageUri    = p.imageUri;
        meta.twitter     = p.twitter; meta.telegram    = p.telegram; meta.website     = p.website;
    ...
    function _graduateToken(address token) internal {
        ...
        if (pooledUsdc > kingOfHillRaised) {
            kingOfHill = token; kingOfHillRaised = pooledUsdc;
    ...
    function _emitMilestones(address token, uint256 raised) internal {
        ...
        uint256 progress = (raised * BPS_DENOMINATOR) / graduationThreshold;
    ...
    function _validateBuySize(TokenState storage state, uint256 tokensOut, address token) internal view {
        ...
        if (tokensOut > state.curveTokens - state.realTokensSold || IERC20(token).balanceOf(address(this)) < tokensOut) revert InvalidAmount();
    }

    function _calcBuyFees(uint256 usdcIn, uint256 createdAt_) internal view returns (BuyFees memory f) {
        if (block.timestamp < createdAt_ + antiSnipeDuration) f.snipeTax = (usdcIn * antiSnipeTaxBps) / BPS_DENOMINATOR;
    ...
    function _checkBlacklist(address token, address wallet) internal view { if (blacklistedTokens[token] || blacklistedWallets[wallet]) revert Blacklisted(); }
    function _priceFromReserves(uint256 uR, uint256 tR) internal pure returns (uint256) { return (uR * 1e30) / tR; }
    function _getBuyAmount(uint256 uR, uint256 tR, uint256 uIn) internal pure returns (uint256) { return (tR * uIn) / (uR + uIn); }
    function _getSellAmount(uint256 uR, uint256 tR, uint256 tIn) internal pure returns (uint256) { return (uR * tIn) / (tR + tIn); }
